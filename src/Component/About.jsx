...
import Cupid from "../Material/cupid.jpeg";
const About = ({ header, detail, styleforFirstDiv, styleForSecondDiv, color,textWidth }) => {
  return (
    <div className={`${color ? color : "bg-pink-500"} `}>
      <div className="grid lg:p-16 md:p-10 p-5 grid-cols-12 container mx-auto  ">
        <div className={`lg:col-span-8  md:col-span-8 col-span-12 ${styleforFirstDiv}`}>
          <h1 className=" lg:text-5xl text-4xl  font-semibold text-primary  ">
           {header?header:"About Cupid Club"} 
          </h1>
          <p className={`${textWidth?textWidth:"lg:w-[90%] md:w-[90%] w-full"} font-semibold text-md pt-10  `}>
            {detail
              ? detail
        ...
        <div className={`lg:col-span-4 md:col-span-4 col-span-12 col-span-12 md:pt-0 pt-10  ${styleForSecondDiv} `}>
          <img src={Cupid} alt="Cupid" style={{ width: "500px" }} />
        </div>
      </div>
    </div>
  );
